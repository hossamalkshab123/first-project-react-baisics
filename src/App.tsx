...
function App() {
  ...
  <label className='block font-medium text-gray-600 my-2' htmlFor={input.id} key={input.id}>
    {input.label}
  ...
  const renderFormInputsToEdit = (id:string,label:string,name: ProductNameTypes) => {
    return (
      <label className='block font-medium text-gray-600 my-2' htmlFor={id}>
      {label}
  ...
  return (
    <>
      ...
      <span className='text-2xl text-blue-950 w-fit font-mono italic font-semibold'>Car Shop</span>
      </header>

      <main className='container'>
      <div className='flex items-center justify-between bg-gray-100 py-3 px-5 rounded-md'>
        <h1 className='text-2xl sm:text-3xl text-blue-950 font-mono  font-semibold '>Latest Products</h1>
        <Button className='w-fit bg-blue-900 hover:bg-blue-700' onClick={()=> openModal()}>ADD A NEW PRODUCT</Button>
      </div>
